...
SELECT * FROM spectrum.hudi_trips_kinesis_streaming_table_ts limit 10;

--Total number of entries

select count (tripId) as numOfTrips from spectrum.hudi_trips_kinesis_streaming_table
...
from spectrum.hudi_trips_kinesis_streaming_table_ts limit 10;

--Number of trips for each route

select routeId, count(tripId) as numOfTrips
from spectrum.hudi_trips_kinesis_streaming_table_ts
group by routeId
order by routeId;
